...
//		int[][] tabuleiroInicial = gt.gerarTabuleiro();
		...
		System.out.println("<<< TESTE HEURISTICA MANHATTAN >>>");
		for (int teste = 0; teste < QNTTESTE; teste++) {
			...
			SolucaoHeuristica busca = null;


			busca = new SolucaoHeuristica(problemaInicial, maximoDeFilhosGerados);

			long start = System.currentTimeMillis();
			solucao = busca.heuristicaManhattan();
		...
		System.out.println("<<< TESTE HEURISTICA FORA DE POSIÇÃO >>>");
		for (int teste = 0; teste < QNTTESTE; teste++) {
			...
			long start = System.currentTimeMillis();
			solucao = busca.heuristicaForaDePosicao();
			long end = System.currentTimeMillis();
