...
import br.edu.ufpi.ia.jogodos8.buscas.SolucaoCega;
...
//		int[][] tabuleiroInicial = gt.gerarTabuleiro();
		...
		System.out.println("<<< TESTE BUSCA EM LARGURA >>>");
		for (int teste = 0; teste < QNTTESTE; teste++) {
			...
			SolucaoCega busca = null;


			busca = new SolucaoCega(problemaInicial, maximoDeFilhosGerados);

			long start = System.currentTimeMillis();
			solucao = busca.largura();
...
//			}
		...
		System.out.println("<<< TESTE PROFUNDIDADE >>>");
		for (int teste = 0; teste < QNTTESTE; teste++) {
